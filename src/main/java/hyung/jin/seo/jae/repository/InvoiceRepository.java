...
public interface InvoiceRepository extends JpaRepository<Invoice, Long>{  
    ...
    @Query("SELECT new hyung.jin.seo.jae.dto.InvoiceDTO(i.id, i.credit, i.discount, i.paidAmount, i.amount, i.registerDate, i.payCompleteDate) FROM Invoice i WHERE i.id = (SELECT MAX(en.invoice.id) FROM Enrolment en WHERE en.student.id = ?1 AND en.old = false)")
